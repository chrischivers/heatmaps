...
import static googleutils.MathUtil.*;
...
public class SphericalUtil {
    ...
    public static LatLng computeOffsetOrigin(LatLng to, double distance, double heading) {
        ...
        double n4 = sin(toRadians(to.lat));
        ...
        double fromLngRadians = toRadians(to.lng) -
    ...
    static double computeSignedArea(List<LatLng> path, double radius) {
        ...
        double prevTanLat = tan((PI / 2 - toRadians(prev.lat)) / 2);
        double prevLng = toRadians(prev.lng);
        // For each edge, accumulate the signed area of the triangle formed by the North Pole
        // and that edge ("polar triangle").
        for (LatLng point : path) {
            double tanLat = tan((PI / 2 - toRadians(point.lat)) / 2);
            double lng = toRadians(point.lng);
